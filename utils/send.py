...
async def send_who_search_message(age):
    ...
    await send_message(
        _('🛑СТОП🛑\n'
          'Это очень важный пункт, требуется чуть больше ‼️ВНИМАНИЕ‼️, ведь в зависимости от того, что ты выберешь, '
          'будет 2 варианта событий:\n\n'
...
async def send_about_yourself_message(keyboard=None):
    ...
    await send_message(
        _('Расскажи о себе 👤\n'
          'Приподнеси себя так, чтобы другой человек мог заинтересоваться тобой. '
          'Сделай это так, чтобы тебе было самому приятно читать свое описание! Но помни, что здесь ты ищешь не '
          'вторую половинку, а друзей и подруг, с которыми у тебя может получится что-то большее) '
...
async def send_hobby_message(keyboard=None):
    ...
    await send_message(
        _('Какие у тебя хобби? Напиши их через запятую, чтобы человек смог зацепиться за них взглядом, например: '
          '«Бассейн, футбол, Формула 1, крипта» 🎨 '),
        reply_markup=keyboard)
...
async def send_first_introduction_message():
    ...
    await send_message(
        _('Я создан энтузиастами, которые против того, чтобы ты платил за возможность лайкнуть кого-то 😉, '
          ...
          'любим TINDER/BADOO за то, что там требуется оплата, чтобы увидеть кто вас лайкнул🤨.'),
        reply_markup=keyboard)
